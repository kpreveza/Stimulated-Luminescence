...
def tl(x,Im,Tm,E,R):
	'''Theoretical function for the TL phenomenon'''
	c=(1-R)/R
...
def freq_f(Tm,E,R):
	'''Function for the computation of the frequency factor'''
	c=(1-R)/R
	F_m=((K*(Tm**2))/E)*np.exp(-E/(K*Tm))*(1-((2*K*Tm)/E))
	z_m=1/c-np.log(c)+(E*np.exp(E/(K*Tm))*F_m)/(K*(Tm**2)*(1-1.05*(R**1.26)))
	w=np.real(lambertw(np.exp(z_m)))
	return ((1-R)*(1+w)**2*heat_rate*E)/((1+2*w)*(K*Tm**2*np.exp(-E/(K*Tm))))


def make_model(num):
	'''We define the model using the theoretical function and define the parameters'''
	pref = "tl{}_".format(num)
	peak = list_peaks[num]
	model = Model(tl, prefix = pref)
	...
	model.set_param_hint(pref+'E', value = c[0][num], min = c[1][num], max = c[2][num])
	model.set_param_hint(pref+'R', value = d[0][num], min = d[1][num], max = d[2][num])
	return model
...
def loop_init(name1,name2,name3,num):
	...
	name2=[]
	name3=[]
	for i in range(num,len(datap),5):
...
if b_sequence[0]=='exp':
	...
	c = loop_init('Einit','minE','maxE',3)
	d = loop_init('Rinit','minR','maxR',4)
...
fieldNames = ['Number of peaks',"Start number",'End number','Heating rate']
fieldValues = []
...
'''Creating new directories to save seperately the output files'''
...
for i in range(0,len(dt),4):
	Ef.append(values[i])
for i in range(1,len(dt),4):
	If.append(values[i])
for i in range(2,len(dt),4):
	Rf.append(values[i])
for i in range(3,len(dt),4):
	Tf.append(values[i])

'''Frequency factor calculation'''
frequency_factor=[]
for i in range(peaks):
	s=freq_f(Tf[i],Ef[i],Rf[i])
	frequency_factor.append(s)


df = pd.DataFrame(list(zip(name_peaks,If,Tf,Ef,Rf,frequency_factor,Fom)),columns =['Peaks', 'Imax','Tmax','E','R','s','Total Fom'])
#df=df.round(3)
df.to_csv(file_exist("Fitted parameters/{}".format(newfile2)),sep='\t')

'''New dataframe with fitted parameters thw will be used for the seperate peak files'''
dk=pd.DataFrame(list(zip(name_peaks,If,Tf,Ef,Rf,frequency_factor)),columns =['Peaks', 'Imax','Tmax','E','R','s'])
dk=dk.round(3)


'''Peaks to seperate files'''
	...
	for i in range(peaks):
		if exists('Fitted parameters/Peak_{}_{}.txt'.format(i+1,file1))==True:
			with open ('Fitted parameters/Peak_{}_{}.txt'.format(i+1,file1), 'a') as filep:
				filep.write('\n')
				filep.write('{}'.format(i)+'  '+str(dk.loc[i,'Peaks'])+'  '+str(dk.loc[i,'Imax'])+'  '+str(dk.loc[i,'Tmax'])+'  '+str(dk.loc[i,'E'])+'  '+str(dk.loc[i,'R'])+'  '+str(dk.loc[i,'s'])+'  '+str(dk.loc[i,'Condition']))
		else:
...
#plt.savefig('G:/My Drive/Paper LED 2021/TL/tldata.eps', format='eps')