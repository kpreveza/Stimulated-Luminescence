		#########################################
		#                                       #
		#   Deconvolution of CW-OSL curves      #
		#                                       #
...
def make_model(num):
	pref = "cw{}_".format(num)
	peak = list_components[num]
	model = Model(cw, prefix = pref)
	model.set_param_hint(pref+'Im', value = a[0][num], min = a[1][num], max = a[2][num])
	model.set_param_hint(pref+'l', value = b[0][num], min = b[1][num], max = b[2][num])
...
'''User tells the number of components, the start+end of the deconvolution and the heating rate for the calculation of the frequency factor'''
msg = "Write down how many components you want to use, the start and end number."
title = "Data for deconvolution"
fieldNames = ['Number of components',"Start number",'End number']
fieldValues = []
fieldValues = multenterbox(msg,title,fieldNames)

components=int(fieldValues[0])
list_components=range(components)
...
for i in range(1,components+1):
	name_components.append('Component_{}'.format(i))
...
df = pd.DataFrame(list(zip(name_components,If,lf,Rf,Fom)),columns =['Components', 'Imax','lambda','R','Total Fom'])
df=df.round(3)
...
dk=pd.DataFrame(list(zip(name_components,If,lf,Rf)),columns =['Components', 'Imax','lambda','R'])
dk=dk.round(3)
...
if seper_file=='No':
	...
	for i in range(peaks):
		if exists('Fitted parameters/Component_{}_{}.txt'.format(i+1,file1))==True:
			with open ('Fitted parameters/Component_{}_{}.txt'.format(i+1,file1), 'a') as filep:
				filep.write('\n')
				filep.write('{}'.format(i)+'  '+str(dk.loc[i,'Components'])+'  '+str(dk.loc[i,'Imax'])+'  '+str(dk.loc[i,'lambda'])+'  '+str(dk.loc[i,'R'])+'  '+str(dk.loc[i,'Condition']))
		else:
			with open ('Fitted parameters/Component_{}_{}.txt'.format(i+1,file1), 'w') as filep:
...
for i in range(components):
	ytl=da["Model(cw, prefix='cw{}_')".format(i)].to_numpy()
	plt.plot(x,ytl,label='Component {}'.format(i+1))
...
#plt.savefig('G:/My Drive/Paper LED 2021/CW OSL/cwosl_data.eps', format='eps')